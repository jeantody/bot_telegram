...
@pytest.mark.asyncio
async def test_voip_service_alerts_on_ami_warning_even_when_sip_ok(tmp_path: Path) -> None:
    store = BotStateStore(str(tmp_path / "state.db"))
    bot = FakeBot()
    provider = FakeVoipProvider(
        result=VoipProbeResult(
            ...
            target_number="1102",
            hold_seconds=5,
            setup_latency_ms=450,
            total_duration_ms=3100,
            sip_final_code=183,
            error=None,
            started_at_utc="2026-02-16T10:00:00+00:00",
            finished_at_utc="2026-02-16T10:00:03+00:00",
            summary={
                "deviation_alert": False,
                "deviation_reasons": [],
                "ami_warning": True,
                "ami_warning_reason": "AMI timeout ao consultar SIPpeers",
            },
        )
    )
    service = VoipProbeService(
    ...
    assert len(bot.messages) == 1
    assert "AMI WARN" in bot.messages[0]["text"]
    assert "AMI timeout ao consultar SIPpeers" in bot.messages[0]["text"]
    events = store.list_audit_events(limit=5)
    assert events[0]["severity"] == "alerta"
