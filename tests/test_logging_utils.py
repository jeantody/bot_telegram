...
def test_configure_logging_redacts_bearer_authorization(capsys) -> None:
    root_logger = logging.getLogger()
    old_handlers = list(root_logger.handlers)
    old_level = root_logger.level
    try:
        configure_logging("INFO")
        logging.getLogger("httpx").info(
            "Authorization: Bearer abc123.super-secret-token"
        )
        for handler in logging.getLogger().handlers:
            try:
                handler.flush()
                ...
                pass
        captured = capsys.readouterr().out
        assert "abc123.super-secret-token" not in captured
        assert "Authorization: Bearer <redacted>" in captured
    finally:
        root_logger.handlers.clear()
        for handler in old_handlers:
            root_logger.addHandler(handler)
